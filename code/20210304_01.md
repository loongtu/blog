...
上面的程序在编译、运行阶段都没有问题，但是由malloc分配的内存却无法回收。可以利用valgrind定位内存泄漏的原因。
...
int main()
{
    ...
    getheap(p); // 值传递，将NULL传递给形参

    p[0] = 10; // 相当于给NULL指针赋值，Segmentation fault (core dumped)
    p[1] = 20;
    printf("p[0]=%d,p[1]=%d\n", p[0], p[1]);
...
上述代码段在运行中时，若fun2内部出现错误，free函数不能正常执行，所以出现内存泄漏的情况。
